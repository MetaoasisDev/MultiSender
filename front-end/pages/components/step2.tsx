...
import fujiConfig from '../config/fuji.json';
import bscConfig from '../config/bsc.json';
import cchainConfig from '../config/cchain.json';
...
const contracts: contractAddressObj = {
    ...
    cchain : cchainConfig.sender,
    fuji : fujiConfig.sender,
    // Testnet Configs
...
export default function Step2(props:Iprops) {
    ...
    return <Box>
        <div className="mb-3">
            ...
            <button onClick={setGasFee}>가스비 체크 시작</button>
            <div>현제 가스가격 : {gasPrice} </div>
            <TableBox>
